...
class JetVerzendt_Shipping_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        $value           = false;
        $shippingMethods = Mage::getStoreConfig(self::STORE_CONFIG_PATH_SHIPPING_METHODS, Mage::app()->getStore());
    ...
    {
        return Mage::getStoreConfig(self::STORE_CONFIG_PATH_SHIPPING_METHODS, Mage::app()->getStore());
    }
    ...
    {
        return Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_ENABLE, Mage::app()->getStore());
    }
    ...
    {
        return Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_DELIVERY_DHL_ENABLE, Mage::app()->getStore());
    }
    ...
    {
        return Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_SATERDAY_DPD_ENABLE, Mage::app()->getStore());
    }
    ...
    {
        return Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_DPD_PARCELSHOP_ENABLE, Mage::app()->getStore());
    }
    ...
    {
        return Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_DHL_PARCELSHOP_ENABLE, Mage::app()->getStore());
    }
    ...
    {
        return Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_FADELLO_ENABLE, Mage::app()->getStore());
    }
    ...
    {
        // check deliver day
        $deliverDay = Mage::getModel('core/date')->date('Ymd', strtotime($input));

        $dailyDeliveryTime = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_DELIVERY_DHL_TIME, Mage::app()->getStore());
        $dailyDeliveryTime = (isset($dailyDeliveryTime) && ! empty($dailyDeliveryTime)) ? str_replace(',', '', $dailyDeliveryTime) : '000000';

        // get next deliver day
        switch (Mage::getModel('core/date')->date('w')) {
            case 6: // saterday
                $nextDeliverDay = Mage::getModel('core/date')->date('Ymd', strtotime('+3 days'));
                break;
            case 0: // sunday
                $nextDeliverDay = Mage::getModel('core/date')->date('Ymd', strtotime('+2 days'));
                break;
            default: // other days
                $nextDeliverDay = Mage::getModel('core/date')->date('Ymd', strtotime('+1 day'));
                break;
        }
    ...
    {
        $deliveryTime = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_SATERDAY_DPD_TIME, Mage::app()->getStore());
        $deliveryTime = (isset($deliveryTime) && ! empty($deliveryTime)) ? str_replace(',', '', $deliveryTime) : '000000';
    ...
    {
        $deliveryTime = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_FADELLO_TIME, Mage::app()->getStore());
        $deliveryTime = (isset($deliveryTime) && ! empty($deliveryTime)) ? str_replace(',', '', $deliveryTime) : '000000';
    ...
    {
        return Mage::getModel('jetverzendt_shipping/package')->loadByOrderId($orderId);
    ...
    {
        $shipmentShippers = Mage::getStoreConfig(self::STORE_CONFIG_PATH_SHIPPING_SHIPPERS, Mage::app()->getStore());
    ...
    public function sendOrdersToJetVerzendt($order, $postData)
    {
        ...
        $token = Mage::getStoreConfig(self::STORE_CONFIG_PATH_TOKEN, Mage::app()->getStore());
        if ( ! $token) {
            Mage::log("Vul uw JetVerzendt autorisatiegegevens correct in", null, 'jetverzendt.log');
            exit;
        }

        // check auto tracktrace mail option
        $autoTrackTrace = (Mage::getStoreConfig(self::STORE_CONFIG_PATH_AUTO_TRACK_TRACE, Mage::app()->getStore())) ? true : false;

        // format streetline data
        $street = $this->splitStreet($order->getShippingAddress()->getData('street'));

        $shipmentData                   = array();
        $shipmentData['company_name']   = $order->getShippingAddress()->getCompany();
        $shipmentData['street_line_1']  = $street[0];
        $shipmentData['number_line_1']  = $street[1];
        $shipmentData['zip_code']       = $order->getShippingAddress()->getPostcode();
        $shipmentData['city']           = $order->getShippingAddress()->getCity();
        $shipmentData['country']        = $order->getShippingAddress()->getCountry();
        $shipmentData['contact_person'] = $order->getShippingAddress()->getFirstname() . ' ' . $order->getShippingAddress()->getMiddlename() . ' '
        ...
        $shipmentData['pickup_date']  = (isset($postData['pickup_date']) && ! empty($postData['pickup_date'])) ? date(
            'Y-m-d', strtotime($postData['pickup_date'])
        ) : Mage::getModel('core/date')->date('Y-m-d', strtotime("+1 day"));
        $shipmentData['product']      = (isset($postData['product']) && $postData['product']) ? $postData['product'] : '';
        $shipmentData['service']      = (isset($postData['service']) && $postData['service']) ? $postData['service'] : '';
        $shipmentData['product_type'] = '2';
        $shipmentData['amount']       = (isset($postData['amount']) && $postData['amount']) ? $postData['amount'] : '';

        if (isset($postData['reference']) && ! empty($postData['reference'])) {
            $shipmentData['reference'] = $postData['reference'];
        } elseif (isset($postData['increment_id']) && ! empty($postData['increment_id'])) {
            $shipmentData['reference'] = $postData['increment_id'];
        } else {
            $shipmentData['reference'] = '';
        }
        $shipmentData['predict']                    = (isset($postData['predict']) && $postData['predict']) ? $postData['predict'] : '';
        $shipmentData['cod']                        = (isset($postData['cod']) && $postData['cod']) ? number_format($postData['cod'], 2, ',', '')
            : '';
        $shipmentData['saturday_delivery']          = (isset($postData['saturday_delivery']) && $postData['saturday_delivery']) ? 1 : 0;
        $shipmentData['weight']                     = (isset($postData['weight']) && $postData['weight']) ? $postData['weight'] : '';
        $shipmentData['signature_required']         = (isset($postData['signature_required']) && $postData['signature_required']) ? 1 : 0;
        $shipmentData['not_by_neighbours']          = (isset($postData['not_by_neighbours']) && $postData['not_by_neighbours']) ? 1 : 0;
        $shipmentData['evening']                    = (isset($postData['evening']) && $postData['evening']) ? 1 : 0;
        $shipmentData['extra_insurance']            = (isset($postData['extra_insurance']) && $postData['extra_insurance']) ? number_format(
            $postData['extra_insurance'], 2, ',', ''
        ) : '';
        $shipmentData['insurance']                  = (isset($postData['insurance']) && $postData['insurance']) ? number_format(
            $postData['insurance'], 2, ',', ''
        ) : '';
        $shipmentData['parcel_shop_id']             = (isset($postData['parcel_shop_id']) && $postData['parcel_shop_id']) ? $postData['parcel_shop_id'] : '';
        $shipmentData['send_track_and_trace_email'] = $autoTrackTrace;

        $apiStatus = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LIVE_STATUS, Mage::app()->getStore());
        if ($apiStatus) {
            $ch = curl_init('https://portal.jetverzendt.nl/api/v2/shipment?api_token=' . $token);
        } else {
            $ch = curl_init('http://testportal.jetverzendt.nl/api/v2/shipment?api_token=' . $token);
        }
    ...
    private function getUserDetails(Mage_Sales_Model_Order $order)
    {
        ...
        foreach ($order->getAddressesCollection() as $address) {
            ...
            if (($addressType == 'shipping' || $addressType == 'billing') && ! $address->isDeleted()) {
    ...
    public function retrieveOrderFromJetVerzendt($order)
    {
        ...
        if ($apiStatus) {
            $ch = curl_init('https://portal.jetverzendt.nl/api/v2/shipment/' . $order->getJetShipmentId() . '?api_token=' . $token);
        } else {
            $ch = curl_init('http://testportal.jetverzendt.nl/api/v2/shipment/' . $order->getJetShipmentId() . '?api_token=' . $token);
        }
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt(
    ...
    public function retrieveLabelsFromJetVerzendt($jetIds)
    {
        ...
        $printerMethod = Mage::getStoreConfig(self::STORE_CONFIG_PATH_PRINTER_METHODS, Mage::app()->getStore());
        $printerLabel  = Mage::getStoreConfig(self::STORE_CONFIG_PATH_PRINTER_LABEL, Mage::app()->getStore());
        ...
        $apiStatus = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LIVE_STATUS, Mage::app()->getStore());
        if ($apiStatus) {
            $ch = curl_init('https://portal.jetverzendt.nl/api/v2/label?api_token=' . $token);
        } else {
            $ch = curl_init('http://testportal.jetverzendt.nl/api/v2/label?api_token=' . $token);
        }
        ...
        $labelData = json_encode(
            array(
                'shipments' => $jetIds,
                'label'     => array('type' => strtoupper($printerMethod),
    ...
    public function getParcelShops($address)
    {
        ...
        $apiStatus = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LIVE_STATUS, Mage::app()->getStore());
        if ($apiStatus) {
            $ch = curl_init('https://portal.jetverzendt.nl/api/v2/parcel-shop/search?api_token=' . $token);
        } else {
            $ch = curl_init('http://testportal.jetverzendt.nl/api/v2/parcel-shop/search?api_token=' . $token);
        }

        $products                      = array();
        $isLastmileDpdParcelshopEnable = Mage::helper('jetverzendt_shipping')->isLastmileDpdParcelshopEnable();
        $isLastmileDhlParcelshopEnable = Mage::helper('jetverzendt_shipping')->isLastmileDhlParcelshopEnable();
    ...
    public function getDeliverDates($address)
    {
        ...
        $apiStatus = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LIVE_STATUS, Mage::app()->getStore());
        if ($apiStatus) {
            $ch = curl_init('https://portal.jetverzendt.nl/api/v2/delivery-schedule/search?api_token=' . $token);
        } else {
            $ch = curl_init('http://testportal.jetverzendt.nl/api/v2/delivery-schedule/search?api_token=' . $token);
        }
    ...
    {
        $price = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_PRICE_DHL_EVENING, Mage::app()->getStore());
        if (isset($price)) {
    ...
    {
        $price = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_SATERDAY_DPD_PRICE, Mage::app()->getStore());
        if (isset($price)) {
            return str_replace(',', '.', $price);
        }
    ...
    {
        $price = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_PRICE_DHL_PARCELSHOP, Mage::app()->getStore());
        if (isset($price)) {
            return str_replace(',', '.', $price);
        }
    ...
    {
        $price = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_PRICE_DPD_PARCELSHOP, Mage::app()->getStore());
        if (isset($price)) {
            return str_replace(',', '.', $price);
        }
    ...
    {
        $price = Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_FADELLO_PRICE, Mage::app()->getStore());
        if (isset($price)) {
            return str_replace(',', '.', $price);
        }
    ...
    {
        return Mage::getStoreConfig(self::STORE_CONFIG_PATH_LASTMILE_GOOGLE_MAPS_KEY, Mage::app()->getStore());
    }
    ...
    {
        return Mage::getModel('checkout/session')->getQuote()->getShippingAddress()->getCountry();
